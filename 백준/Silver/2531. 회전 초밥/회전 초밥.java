...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int N = Integer.parseInt(st.nextToken()); // 회전 초밥 개수
        int d = Integer.parseInt(st.nextToken()); // 회전 초밥 가지 수
        int k = Integer.parseInt(st.nextToken()); // 연속으로 먹었을 때 할인 적용 되는 수
        ...
        for(int i=0; i<N; i++){
            sushi[i] = Integer.parseInt(br.readLine()); // 초밥 정보 입력 받기
        }
        ...
        int max = kinds!=k? kinds:cnt[c]==0? kinds+1:kinds; // 처음 윈도우의 최대값을 구해서 최초 최대값 선언
